...
Local $downDownload = InetGet($LauncherURL, $Launcherpath, 1, $INET_DOWNLOADWAIT)

; Prüfen ob der Download erfolgreich war
If $downDownload Then
    ; Überprüfen, ob die Datei existiert
    If FileExists($Launcherpath) Then
        ; Optional: Überprüfen der Dateigröße (falls bekannt)
        $expectedFileSize = 123456  ; Beispielgröße in Bytes
        $downloadedFileSize = FileGetSize($Launcherpath)
        If $downloadedFileSize = $expectedFileSize Then
            WriteLog("Download Launcher abgeschlossen" & @CRLF)
        Else
            WriteLog("Download abgeschlossen, aber Dateigröße ist inkorrekt: " & $downloadedFileSize & " Bytes" & @CRLF)
        EndIf
    Else
        WriteLog("Download fehlgeschlagen: Datei nicht gefunden" & @CRLF)
    EndIf
Else
    WriteLog("Download fehlgeschlagen: InetGet Fehler" & @CRLF)
EndIf
